...
class TalleresTableInscripcion extends JTable
{
    /**
     * Constructor
     *
    ...
    {
        parent::__construct('#__inscripcion', 'id', $db);
    }
...
//        }
    ...
    public function check()
    {
        ...
        $query->select('id');
        $query->from($db->quoteName('#__inscripcion'));
        $query->where('taller_id = ' . $this->taller_id . ' AND ci = ' . $this->ci);
        $db->setQuery($query);
        $xid = intval($db->loadResult());
        if ($xid && $xid != intval($this->id)) {
            //$db->
            $this->setError(JText::_('COM_TALLERES_MSG_INSCRIPCION_EXISTE'));
            return false;
        }
