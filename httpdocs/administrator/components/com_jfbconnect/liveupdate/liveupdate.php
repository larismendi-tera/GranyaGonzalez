...
class LiveUpdate
{
	...
	public static function getIcon($config=array())
	{
		...
		if(!$updateInfo->supported) {
            ...
            return self::getIconHtml($url, $img, $class, $lbl);
    ...
    static function getIconHtml($url, $img, $class, $lbl)
    {
        return '<div class="icon"><a href="'.$url.'">'.
        			'<div style="float:left; margin-left:5px"><img src="'.$img.'" width="32" height="32" border="0" align="middle" style="float: none" /></div>'.
        			'<span style="float:left" class="'.$class.'">'.$lbl.'</span></a></div>';
    }
}