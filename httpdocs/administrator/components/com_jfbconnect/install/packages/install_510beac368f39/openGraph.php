...
class OpenGraphTags
{
    var $version = "1.0.0"; // Same as the library XML version
    ...
    public function getOpenGraphTags($graphEasyTags)
    {
        ...
        if ($this->isOpenGraphEnabled == '1')
        {
            ...
            if (strpos($headerGraphString, 'fb:app_id') === false && $this->componentType == SOURCECOAST_JFBCONNECT)
            {
                //Add in App ID
                if ($this->componentAppId)
                    $headerGraphString .= '<meta property="fb:app_id" content="' . $this->componentAppId . '"/>' . CARRIAGE_RETURN;
            }
