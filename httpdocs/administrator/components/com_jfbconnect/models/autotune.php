...
class JFBConnectModelAutoTune extends JModel
{
    ...
    public function getFieldDescriptors($forceUpdate = false)
    {
        if (!$this->fieldDescriptors)
        {
            ...
            $checkWindow = strtotime('+7 day', strtotime($updatedAt));
    ...
    */
    ...
    public function isPluginEnabled($name)
    {
            ...
            $query = 'SELECT enabled FROM #__extensions WHERE `type`="plugin" AND `element`="' . $name . '"';
    ...
    public function publishPlugin($name, $status)
    {
            ...
            $query = 'UPDATE #__extensions SET enabled="' . $status . '" WHERE `type`="plugin" AND `element`="' . $name . '"';
    ...
    public function getJoomlaErrors()
    {
        ...
        // Check ordering of System - Cache plugin
            ...
            $query = 'SELECT element, enabled, ordering FROM #__extensions WHERE `type`="plugin" AND (`element`="jfbcsystem" OR `element`="cache") ORDER BY element';
