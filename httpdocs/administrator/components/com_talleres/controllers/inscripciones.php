...
class TalleresControllerInscripciones extends JControllerAdmin
{
    ...
    public function __construct($config = array())
    {
        ...
        $this->registerTask('asistencia_confirm', 'changeAssistance');
        $this->registerTask('asistencia_pending', 'changeAssistance');
    ...
    // @larismendi opcion de asistencia 13-03-2015
    public function changeAssistance() {
        ...
        $values = array('asistencia_confirm' => 1, 'asistencia_pending' => 0);
        $task   = $this->getTask();
        ...
        if (empty($ids)) {
            ...
            if (!$model->changeAssistance($ids, $value)) {
                JError::raiseWarning(500, $model->getError());
            } else {
                if ($value == 1) {
                    $ntext = 'COM_TALLERES_N_ASSISTANCE_CONFIRMED';
                } else {
                    $ntext = 'COM_TALLERES_N_ASSISTANCE_PENDING';
                }
