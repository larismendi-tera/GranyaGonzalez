...
if ( !/*NOT*/JFactory::getUser()->authorise( 'core.manage', 'com_content' ) )
{
...
class JUserportHelper
{
  ...
  public function NotifyUser( $task, $user, $settings, $userLog = array() )
  {
    ...
    if ( strlen( $user->activation ) > 0 )
    {
      ...
      $activationUrl .= "index.php?option=com_users&task=registration.activate&token=" . $user->activation;
    }
    $token = array
    ( '{user_name}' => $user->name,
   ...
   *   extracted.
   * @param ordered-unnamed-array $headerData. It is assumed that the field
   *   types correspond (this is not checked here).
   *   If this value is @c null, the string data is also used as variable names
   *   in the resulting object.
   * @param string $fieldSeparator The character that is used separating
   *   two fields.
   * @param string $fieldEnclosure The character that is used to mark
   *   separating two fields.
   * @param array-of-arrays-of-strings $log Used for logging information
   *   messages, warnings and errors, interesting for the user of userport.
   * @return object. With the headerdData as variable names, and the string
   ...
   * @param object $userData An object containing the user's data, which must be
   *   transformed to a string. If @c null, a line with the field names used
   *   will be constructed, which can serve as a header line.
   * @param string $fieldSeparator A single character that is used to separate
   *   two fields.
   * @param string $fieldEnclosure A single character that is used to mark
   *   the start and the end of a field.
   * @param Array-of-strings $userFields A list of fields to retain; all other
   *   fields are discarded.
   * @param Array-of-array-of-strings $log Used for logging information
   *   messages, warnings and errors, interesting for the user of userport.
   * @return string. Contains the csv version of the given object.
   */
...
/**/self::$_sAllUserFields[] = array( 'id', 'name', 'username', 'email', 'password', 'groups', 'block', 'sendEmail', 'registerDate', 'lastvisitDate', 'activation', 'params' );
  ...
  protected function ReadFile( $filename, & $log )
  {
    ...
    /* MS Notepad prepends a BOM when switching the encoding from ANSI to UTF-8:
     * this looks like whitespace, but has byte sequence 0xEF 0xBB 0xBF
     * Also, trim any ASCII control characters as well.
     */
    $text = ltrim( $text, "\x00..\x1F");
   ...
   * @param array-of-arrays-of-strings $log Used for logging information
   *   messages, warnings and errors, interesting for the user of userport.
   * @return string. The contents of the file when all went succesfull.
   */
   ...
   * @param string $string The string version of a registry object
   * @return JRegistry
   */
