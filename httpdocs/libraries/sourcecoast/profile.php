...
class SCProfileLibrary extends JPlugin
{
    ...
    function getConfigurationTemplate()
    {
        ...
        $file = JPATH_SITE . DS . 'plugins' . DS . $this->profileDirectory . DS . $this->profileName . DS . $this->profileName . DS . 'tmpl' . DS . 'configuration.php';
         //SC16
    ...
    function migrateSocialAvatarToProfile($joomlaId, $socialUserId)
    {
        ...
        $avatarHost = $avatarParts['host'];
        $avatarPath = $avatarParts['path'];
        $avatarExt = $avatarParts['ext'];
        $fullAvatarPath = $avatarHost .'/'.$avatarPath;

        $baseImgPath = $this->getAvatarPath();
        $tmpImgName = 'scprofile_' . $joomlaId . '_pic_tmp' . $avatarExt;

        $source = '';
        $fp = fsockopen($avatarHost, 80, $errno, $errstr, 30);

        if ($fp)
        {
            //$out = "GET $avatarPath HTTP/1.1\r\n";
            //$out .= "Host: " . $avatarHost . "\r\n";
            $out = "GET $fullAvatarPath HTTP/1.1\r\n";
