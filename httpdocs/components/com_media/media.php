...
//If user is logged in and is front end

$app	= JFactory::getApplication();
if ($app->isSite() && $user->id > 0) {

    $userMediaBase = JPATH_ROOT.'/'.$params->get('image_path', 'images'). DIRECTORY_SEPARATOR . 'userdata';

    if (!is_dir($userMediaBase)) {
        mkdir($userMediaBase);
    }

    $userMediaBase .=  DIRECTORY_SEPARATOR .  $user->id;

    if (!is_dir($userMediaBase)) {
        mkdir($userMediaBase);
    }

    $userMediaBaseUrl = JURI::root().'/'.$params->get('image_path', 'images') . '/userdata/' . $user->id;

    define('COM_MEDIA_BASE', $userMediaBase);
    define('COM_MEDIA_BASEURL', $userMediaBaseUrl);
    define('COM_MEDIA_RELATIVE_BASEURL', $params->get('image_path', 'images') . '/userdata/' . $user->id . '/');
} else {
    ...
    define('COM_MEDIA_RELATIVE_BASEURL', $params->get('image_path', 'images') . '/');
}
