...
class ContactControllerContact extends JControllerForm
{
    ...
    private function _sendEmail($data, $contact)
    {
        ...
        $body = $body . "\n" . 'País: '.$this->getCountryNameById($data['contact_country']);
        $body = $body . "\n" . 'Ciudad: '.$data['contact_city'];
        if ($data['contact_skype']!='') {
            $body = $body . "\n" . 'Skype: '.$data['contact_skype'];
        }
        /* Custom, get emails from address field */
        $emails = explode(',', $contact->address);
        if (count($emails) > 0) {

            $mail = JFactory::getMailer();
            foreach ($emails as $dir) {
                if (filter_var($dir, FILTER_VALIDATE_EMAIL)) {
                    $mail->addRecipient(trim($dir));
                }
            }
    ...
    private function getCountryNameById($id) {
        ...
        $query->select('name')->from('#__countries')->where('id='.intval($id));
        $db->setQuery($query);
